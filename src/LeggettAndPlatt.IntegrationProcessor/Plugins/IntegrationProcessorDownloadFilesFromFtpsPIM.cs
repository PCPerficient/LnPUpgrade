...
{
    public class IntegrationProcessorDownloadFilesFromFtpsPIM : IIntegrationProcessor
    {
        ...
        private DataSet DownloadFiles(SiteConnection siteConnection, IntegrationJob integrationJob, JobDefinitionStep jobStep)
        {
            ...
            this.JobLogger.Info("Download files form FTPS job start");

            if (this.IntegrationJob == null)
            {
                this.JobLogger.Fatal("IntegrationJob is Null : Required to execute DownloadFilesFromFtpsPIM");
                throw new ArgumentNullException("integrationJob", "Integration Job required to execute DownloadFilesFromFtpsPIM");
            }
            if (this.IntegrationJob.JobDefinition.JobDefinitionParameters == null)
            {
                this.JobLogger.Fatal("JobDefinitionParameters is Null : Parameter required to execute DownloadFilesFromFtpsPIM");
                throw new ArgumentNullException("integrationJob", "Integration Job Defination Parameter required to execute DownloadFilesFromFtpsPIM");
            }
            ...
            {
                this.JobLogger.Fatal("IntegrationConnection is Null : Required to execute DownloadFilesFromFtpsPIM");
                throw new ArgumentNullException("integrationJob", "Integration connection import folder required to execute DownloadFilesFromFtpsPIM");
            }
  ...
  x => x.JobDefinitionParameter != null
            ...
            ftpRequest.FTPRemoteFolderPath = this.GetParameterValue(
             ...
             JobDefinitionParametersConstant.FTPSRemoteFolderPath,
            ...
            FtpsManager ftpsClient = new FtpsManager(ftpRequest);
            this.JobLogger.Info("Starting reading files from FTP");
            ftpsClient.MoveFilesFromRemoteToLocal();
            ...
            if (downLoadFileStep != null && downLoadFileStep.IntegrationConnectionOverride != null)
            {
                ...
                for (int i = 0; i < addressPort.Length; i++)
                {
                    if (i == 0)
                        ...
                        portNumber = string.IsNullOrEmpty(addressPort[1]) ? 990 : Convert.ToInt32(addressPort[1]);
                }
