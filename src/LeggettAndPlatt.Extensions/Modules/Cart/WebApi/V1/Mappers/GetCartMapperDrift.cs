...
namespace LeggettAndPlatt.Extensions.Modules.Cart.WebApi.V1.Mappers
{
    public class GetCartMapperDrift : GetCartMapper
    {
        ...
        public override CartModel MapResult(GetCartResult serviceResult, HttpRequestMessage request)
        {
            ...
            CartModel cartModel3 = destination;
            DateTimeOffset? requestedPickupDate1 = serviceResult.RequestedPickupDate;
          //  ref DateTimeOffset? local3 = ref requestedPickupDate1;
            string str2;
            if (!requestedPickupDate1.HasValue)
            {
                str2 = (string)null;
            }
            else
            {
                valueOrDefault = requestedPickupDate1.GetValueOrDefault();
                str2 = valueOrDefault.ToString();
            }
            cartModel3.RequestedPickupDate = str2;
            CartModel cartModel4 = destination;
            DateTimeOffset? requestedPickupDate2 = serviceResult.RequestedPickupDate;
         //   ref DateTimeOffset? local4 = ref requestedPickupDate2;
            DateTime? nullable2;
            if (!requestedPickupDate2.HasValue)
            {
                nullable2 = new DateTime?();
            }
            else
            {
                valueOrDefault = requestedPickupDate2.GetValueOrDefault();
                nullable2 = new DateTime?(valueOrDefault.Date);
            }
            cartModel4.RequestedPickupDateDisplay = nullable2;
