...
BEGIN
	...
	WHILE @@FETCH_STATUS = 0 
	BEGIN
		IF (@CategoryId IS NULL OR ISNULL(@ResourceFilename,'') = '' OR ISNULL(@ResourceType,'') = '')
		BEGIN
			BEGIN TRY
				RAISERROR ('Custom Error - Category does not exist in Insite or Resource Type/File Name not provided in PIM', -- Message text.  
							16, -- Severity.  
							1 -- State.  
							)
			END TRY
	...
	WHILE @@FETCH_STATUS = 0 
	BEGIN
		IF (@ProductID IS NULL OR ISNULL(@ResourceFilename,'') = '' OR ISNULL(@ResourceType,'') = '')
		BEGIN
			BEGIN TRY
				RAISERROR ('Custom Error - Product does not exist in Insite or Resource Type/File Name not provided in PIM', -- Message text.  
							16, -- Severity.  
							1 -- State.  
							)
			END TRY
