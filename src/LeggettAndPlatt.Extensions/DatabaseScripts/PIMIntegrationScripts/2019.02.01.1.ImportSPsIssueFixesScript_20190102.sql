...
BEGIN
	...
	DELETE l
		...
		WHERE l.LinkTypeId IN ('ChannelChannelNodes','ChannelNodeChannelNodes')
			...
			AND NOT EXISTS 
				(
					SELECT 1 FROM dbo.PIMChannelNode WHERE EntityId = l.TargetEntityID
				)
...
BEGIN
	...
	DELETE l
		...
		WHERE l.LinkTypeId = 'Product2Items'
			...
			AND NOT EXISTS 
				(
					SELECT 1 FROM dbo.PIMItem WHERE EntityId = l.TargetEntityID
				)
	...
	BEGIN
		...
		BEGIN
			SELECT
					...
					,@ItemSKUStockNumber = 
						CASE 
							WHEN LEN(ISNULL(ItemSKUStockNumber,'')) > 100 THEN LEFT(ItemSKUStockNumber,100) 
							ELSE ItemSKUStockNumber 
						END						-- Sku
					,@UrlSegment = 
						CASE 
							WHEN LEN(ISNULL(ItemSKUStockNumber,'')) > 255 THEN LEFT(ItemSKUStockNumber,255) 
							ELSE ItemSKUStockNumber 
						END						-- UrlSegment
			...
			BEGIN
				...
				BEGIN CATCH
					...
					INSERT INTO #ImportItemLog(LogTypeName,LogMessage,LogDateTime)
						SELECT	'Warn'
								,'Error for Add Item - PIM Entity ID = '+ISNULL(CAST(@EntityID AS NVARCHAR(50)),'**NULL**')
			...
			BEGIN
						...
						SET
							...
							,DeactivateOn = 
								CASE
									...
									ELSE NULL
								END 
							,SortOrder = CASE WHEN @SourceTable = 'Link' THEN @Index ELSE SortOrder END
				...
				BEGIN CATCH
					...
					INSERT INTO #ImportItemLog(LogTypeName,LogMessage,LogDateTime)
						SELECT	'Warn'
								,'Error for Add Item when base product exists for Simple Product - PIM Entity ID = '+ISNULL(CAST(@EntityID AS NVARCHAR(50)),'**NULL**')
			...
			BEGIN	-- insert new row for with item data
				...
				BEGIN
					...
					BEGIN CATCH
						...
						INSERT INTO #ImportItemLog(LogTypeName,LogMessage,LogDateTime)
							SELECT	'Warn'
									,'Error when Adding Item  - PIM Entity ID = '+ISNULL(CAST(@EntityID AS NVARCHAR(50)),'**NULL**')
										+'; PIM Source Entity ID = '+ISNULL(CAST(@SourceEntityID AS NVARCHAR(50)),'**NULL**')
				...
				BEGIN	--@SourceEntityID IS NOT NULL 
					...
					BEGIN CATCH
						...
						INSERT INTO #ImportItemLog(LogTypeName,LogMessage,LogDateTime)
							SELECT	'Warn'
									,'Error when Adding Item for Parent Product - PIM Entity ID = '+ISNULL(CAST(@EntityID AS NVARCHAR(50)),'**NULL**')
										+'; PIM Source Entity ID = '+ISNULL(CAST(@SourceEntityID AS NVARCHAR(50)),'**NULL**')
										+'; Item Name = '+ISNULL(@ItemName,@ItemSKUStockNumber)
										+ '; Error Message = '+@ErrorMessage
									,GETUTCDATE()
				...
				BEGIN
					...
					BEGIN CATCH
						...
						INSERT INTO #ImportItemLog(LogTypeName,LogMessage,LogDateTime)
							SELECT	'Warn'
									,'Error when Adding Content/Custom Property/Document/Kit related data for Item - PIM Entity ID = '
										+ISNULL(CAST(@EntityID AS NVARCHAR(50)),'**NULL**')
										+'; PIM Source Entity ID = '+ISNULL(CAST(@SourceEntityID AS NVARCHAR(50)),'**NULL**')
										+'; Item Name = '+ISNULL(@ItemName,@ItemSKUStockNumber)
										+ '; Error Message = '+@ErrorMessage
									,GETUTCDATE()
		...
		BEGIN
			...
			BEGIN	-- product does not exist in Insite
				...
				BEGIN CATCH
					...
					INSERT INTO #ImportItemLog(LogTypeName,LogMessage,LogDateTime)
						SELECT	'Warn'
								,'Error for Update Item - PIM Entity ID = '+ISNULL(CAST(@EntityID AS NVARCHAR(50)),'**NULL**')
									+'; Item Name = '+ISNULL(@ItemName,'**NULL**')
									+ '; Error Message = '+@ErrorMessage
								,GETUTCDATE()
				END CATCH
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN CATCH
					...
					INSERT INTO #ImportItemLog(LogTypeName,LogMessage,LogDateTime)
						SELECT	'Warn'
								,'Error for Delete Item - PIM Entity ID = '+ISNULL(CAST(@EntityID AS NVARCHAR(50)),'**NULL**')
									+'; Item Name = '+ISNULL(@ItemName,'**NULL**')
									+ '; Error Message = '+@ErrorMessage
								,GETUTCDATE()
				END CATCH
