...
namespace LeggettAndPlatt.Extensions.Plugins.Emails
{
    ...
    //    public EmailServiceDrift(IEmailTemplateUtilities emailTemplateUtilities, IContentManagerUtilities contentManagerUtilities, IEntityTranslationService entityTranslationService, EmailsSettings emailsSettings, Lazy<IEmailTemplateRenderer> emailTemplateRenderer, PinPointeSettings pinPointeSettings, IPinPointeService pinPointeService, EmailHelper emailHelper)
    ...
    //            return;
    //        IQueryable<EmailList> queryable = unitOfWork.GetRepository<EmailList>().GetTable().Expand<EmailList, ICollection<EmailSubscriber>>((Expression<Func<EmailList, ICollection<EmailSubscriber>>>)(x => x.EmailSubscribers));
    //        if (queryable == null || !queryable.Any<EmailList>())
    //            throw new Exception("The email lists were not found");
    //        queryable.ForEach<EmailList>((Action<EmailList>)(el => el.EmailSubscribers.Remove(emailSubscriber)));
